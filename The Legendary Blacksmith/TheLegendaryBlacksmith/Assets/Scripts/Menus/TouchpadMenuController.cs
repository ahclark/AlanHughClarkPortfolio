...
[RequireComponent(typeof(TouchPadRadialMenu))]
[RequireComponent(typeof(HandButtonEvents))]
public class TouchpadMenuController : MonoBehaviour
{
    public HandButtonEvents events;
    [SerializeField]
    GameObject Touchpad;
    [SerializeField]
    bool RightHand = false;
    protected TouchPadRadialMenu menu;
    private float currentAngle; //Keep track of angle for when we click
    private bool touchpadTouched;

    protected virtual void Awake()
    {
        menu = GetComponent<TouchPadRadialMenu>();
    ...
    protected virtual void Initialize()
    {
        if (events == null)
        {
            events = GetComponent<HandButtonEvents>();
        }
    }
    ...
    private void Start()
    {
        if (GameStateManager.managerinstance)
            SetHand(GameStateManager.managerinstance.TeleportLeft);
    }
    ...
    {
        events.hand.controller.TriggerHapticPulse((ushort)(strength * 100));
    }
