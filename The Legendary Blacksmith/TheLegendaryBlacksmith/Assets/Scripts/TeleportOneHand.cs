...
//Written by Ben Ousley, copied from Teleport.cs from the SteamVR script assets
//
//=============================================================================
...
{
    //-------------------------------------------------------------------------
    public class TeleportOneHand : MonoBehaviour
    {
        [SerializeField]
        Hand mHand;
        ...
        //-------------------------------------------------
        private static TeleportOneHand _instance;
        public static TeleportOneHand instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = GameObject.FindObjectOfType<TeleportOneHand>();
                }
        ...
        void Start()
        {
            ...
            if (!mHand)
            {
                if (GameStateManager.managerinstance)
                    GameStateManager.managerinstance.SetTeleportingHand(GameStateManager.managerinstance.TeleportLeft);
            }
        }
        ...
        void Update()
        {
            ...
            if (visible)
            {
                if (WasTeleportButtonReleased(mHand))
                {
                    if (pointerHand == mHand) //This is the pointer hand
                    {
                        TryTeleportPlayer();
                    }
                }
            ...
            if (WasTeleportButtonPressed(mHand))
            {
                newPointerHand = mHand;
            }
        ...
        public void CancelTeleportHint()
        {
            ...
            {
                ControllerButtonHints.HideTextHint(mHand, EVRButtonId.k_EButton_SteamVR_Touchpad);
        ...
        private IEnumerator TeleportHintCoroutine()
        {
            ...
            while (true)
            {
                ...
                bool showHint = IsEligibleForTeleport(mHand);
                bool isShowingHint = !string.IsNullOrEmpty(ControllerButtonHints.GetActiveHintText(mHand, EVRButtonId.k_EButton_SteamVR_Touchpad));
                if (showHint)
                {
                    if (!isShowingHint)
                    {
                        ControllerButtonHints.ShowTextHint(mHand, EVRButtonId.k_EButton_SteamVR_Touchpad, "Teleport");
                    ...
                    if (Time.time > prevHapticPulseTime + 0.05f)
                    {
                        ...
                        mHand.controller.TriggerHapticPulse(500);
                    }
                }
        ...
        public void SetHand(bool left)
        {
            ...
            if (player)
            {
                ...
                {
                    if (player.leftHand)
                        mHand = player.leftHand;
                }
                else
                {
                    if (player.rightHand)
                        mHand = player.rightHand;
                }
            }
            ShowTeleportHint();
        }
